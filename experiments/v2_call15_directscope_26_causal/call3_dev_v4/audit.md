...
**검수 판정: 통과.** `verbatim`은 문자열 동일성이 아니라 식별정보 보존을 뜻하는 것으로
확정했다. 사건번호가 다른 번호로 바뀌거나 주어지지 않은 판례에 번호가 붙으면 여전히 실패다.
정본은 `docs/v2_plan/N_CONFIGURATION_FREEZE.md`이며 같은 자를 P에도 적용한다.
