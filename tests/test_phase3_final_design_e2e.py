...
def test_final_design_gate_preserves_attempt_review_without_forcing_conviction():
    verifier = (ROOT / "scripts/verify_phase3_final_design_e2e.py").read_text(
        encoding="utf-8"
    )
    assert 'relations.get("attempt_to_consider", [])' in verifier
    assert 'relations.get("offense_established", [])' in verifier
    assert 'relations.get("final_offense", [])' in verifier
