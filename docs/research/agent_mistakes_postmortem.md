...
세웠다. 이는 잘못된 권위와 과도한 일반화를 실행 규칙으로 굳힐 수 있다. 최종 정정본은
646개 중 118개를 core 검수 후보로, 528개를 RAG·future-work 문맥으로 분리했고,
118개 전부의 사용자 승인이 끝날 때까지 전체 RuleIR 생성을 차단한다.
...
요건이 아닌 보호법익 다수설 카드도 core에서 제외했다. 최종적으로 deterministic 후보
29개, standard-input 후보 89개, RAG·future-work 528개로 좁혔다. 기망·착오·재산상
...
이미 해소된 결정 지시를 남겼다. 현재는 `candidate`/`review_pending`으로 표시하고
118개가 모두 승인될 때까지 RuleIR을 차단한다.
