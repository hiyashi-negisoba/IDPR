...
2. **재판절차 쟁점 16건(+mixed 1)** — 일부상소·불이익변경금지·상소이익·항소이유서(제출기간·
   추완)·재심·공소기각/형식재판·변호인선임 등. 증거능력 gating(procedure_gating 17)과 **다른
   축**이며, 현행 `admissible`/`provable` gate 아키텍처로 모델링되지 않는 **재판절차** 논점이다.

처리 방침(결정 필요):
- 1번 실체 16건: 8/11까지 rulegen으로 몇 건 커버할지가 A3 본실험 규모를 정한다. paper
  universe 기여도 순으로 우선순위.
- 2번 절차 16건: 논문 scope에서 빼고 coverage tier(§12.2)의 **uncovered로 정직하게 보고**할지,
  별도 재판절차 모델링을 시도할지. 사후 제외가 아니라 tier 명시가 원칙이다.
