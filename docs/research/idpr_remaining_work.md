...
## L5. 루브릭이 부르는데 카드가 없는 형법각칙 12조문 (2026-07-31 추가)

상태: **미착수. Phase 2 리콜 실측 이후에 판단한다.**

L0 골드를 루브릭 기반으로 다시 세우면서(태그 골드 폐기, `data/eval/rubric_crime_article_map.json`)
**KCL 루브릭이 죄명으로 부르는데 카드 코퍼스에 조문이 없는 형법각칙 12개**가 드러났다.
특별법이 아니라 형법각칙이고, 주석서 원문은 전부 확보돼 있다.
...
**원인은 `issue_tags`다.** 캠페인 대상 조문이 태그 매니페스트에서 역산됐는데, 태그는
`scripts/build_kcl_criminal_inventory.py`의 `CURATED_TAGS`에 사람이 문항당 4~5개씩 손으로
써넣은 요약이라, 루브릭 1,166항목이 부르는 죄명 대부분이 타깃에 들어가지 못했다. 같은 원인이
쟁점 리콜 골드도 무효로 만들었다(그래서 골드를 루브릭으로 바꿨다).

메울 대상과 실측 규모 — 합계 **301 chunks ≈ $15.1**(P1 실측 $0.0508/chunk):

| 조문 | 표제 | ch | 조문 | 표제 | ch |
|---|---|---:|---|---|---:|
| 제307조 | 명예훼손 | 94 | 제276조 | 체포·감금 | 31 |
| 제362조 | 장물 | 59 | 제324조 | 강요 | 25 |
| 제283조 | 협박 | 18 | 제266조 | 과실치상 | 15 |
| 제260조 | 폭행 | 14 | 제131조 | 수뢰후부정처사 | 13 |
| 제144조 | 특수공무집행방해 | 13 | 제232조의2 | 사전자기록위작 | 9 |
| 제262조 | 폭행치사상 | 6 | 제174조 | 방화 예비·미수 | 4 |

제307조·제362조는 **번들 96에 이미 들어와 있어 카드만 만들면 된다**(번들에 있으나 카드 없는
조문은 48개). 나머지 10개는 `scripts/extend_commentary_bundle.py`로 원천에서 보강 추출한다 —
제330·332·333조 보강 때 검증된 경로다.

**착수 시점을 뒤로 두는 이유**: 지금 넣으면 미스 목록을 보고 코퍼스를 늘리는 것이 되어,
L0 리콜이 검색 성능이 아니라 코퍼스 조정의 함수가 된다. Phase 2 리콜을 현재 51조문 기준으로
먼저 확정해 보고하고, 그 다음에 커버리지 확장을 독립 항목으로 실행한다. 실행은 예산 게이트
(잔액 확인 + 사용자 승인)를 거친다. 관련: A3, `data/eval/rubric_crime_article_map.json`의
`coverage_gap`.
