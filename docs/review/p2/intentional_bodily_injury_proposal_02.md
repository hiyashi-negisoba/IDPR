...
## 조립 시 join 정규화 (2026-08-04)

초안은 `intent`에 #31·#32를 `mandatory_all`, #29를 `alternative_any`로 두었다. 한 component에
두 join이 섞이면 조립기가 막는다(방화에서 같은 이유로 4건을 고친 전례). 셋 다 같은 요건인
**상해의 고의**를 다른 각도에서 진술한 카드이므로 전부 `alternative_any`로 통일했다.

`mandatory_all`은 결합이므로 셋을 모두 요구하게 된다. 그러면 확정적 고의 사건에서 미필적
고의 카드(#29)까지 충족을 주장해야 성립하는데, 그것은 법리가 아니라 조립 오류다. 어느 한
경로로 고의가 인정되면 요건이 충족되는 `alternative_any`가 맞다.

법률 판단이 바뀐 것은 없다. 세 카드의 role·track·provenance는 승인된 그대로다.
