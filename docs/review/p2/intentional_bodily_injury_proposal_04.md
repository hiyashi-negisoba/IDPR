...
- [x] #88은 `context_only`
- [ ] 제262조 폭행치사상: 아래 실측 참조. 별도 unit 생성 여부 미결

## 제262조 실측 (2026-08-03)

별도 unit을 만들 수 있는지 저장소를 확인한 결과는 다음과 같다.

| 확인 항목 | 제260조 폭행 | 제262조 폭행치사상 |
|---|---|---|
| NormCard | 0장 | 0장 |
| 주석서 rulegen 원천 | 없음 | 없음 |
| KCL 루브릭 gold 등장 | 0문항 | 0문항 |

세 항목 모두 비어 있다. 카드가 없으므로 RuleIR을 조립할 수 없고, 주석서 캠페인 원천도 없어
카드를 만들려면 rulegen 캠페인부터 새로 돌려야 한다. 그리고 폭행치상은 제260조 폭행죄를
기본범으로 하므로 제262조만 만들 수도 없다.

무엇보다 **KCL 59문항의 루브릭 gold에 폭행 계열이 한 번도 등장하지 않는다.** 방화조차 1문항에는
나왔던 것과 비교된다. 즉 지금 만들어도 평가 커버리지 이득은 0이다.
