...
지금은 D4 승인대로 이 unit 안의 track으로 두되, 이관 대상으로 표시해둔다.

## Human decision H-B04

1. 위 71–104번 초안을 일괄 승인하는가? 수정 번호만 적어도 된다.
2. 제263조 동시범 특례를 지금은 이 unit의 track으로 두되 공유 module 이관 대상으로 표시할까?
3. #88을 `context_only`로 두는 데 동의하는가? 특례가 배제하는 원칙이라 규칙으로 넣으면
   특례와 정면으로 충돌한다.
4. 폭행치상·폭행치사(제262조)는 상해 unit의 경계가 자주 향하는 곳인데 현재 범위 밖이다.
   제262조를 별도 unit으로 추가할지, 아니면 계속 `predicate_ir_missing`으로 둘지 판단이 필요하다.
