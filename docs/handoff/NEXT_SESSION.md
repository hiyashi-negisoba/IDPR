...
## 2026-08-14 N 조건 26문항 Call 3 실행 -- 완료

**검수 문서는 `docs/analysis/v2_call3_n_26_run_review_ko.md`다.** 결과와 판정 요청이 거기 있고,
여기에는 상태만 적는다.

동결 지문 그대로 실행했다(sha 3종 일치, 서비스 222907). 26 요청 / 26 기록 / 오류 0.
산출은 `call3_n_26/`.

- F4·F3·식별자 누출·가담 어휘·행위자 생존: **전수 0**. required 105개 누락 0.
- F1/F2 결론 상태 대조: 105 앵커 중 99 일치. 잔여 6건은 **전부 sealed participation 2문항**
  (`r10_p1_q3_ga`, `r11_p1_q1`)에 있고, 확인하려면 본문을 읽어야 하므로 열지 않았다.
- 신규 감사 두 개: `scripts/audit_v2_call3_answer_hygiene.py`(식별자·인용·가담),
  `scripts/audit_v2_call3_conclusion_state.py`(F1/F2 상태 대조).

상태 대조기는 처음 12건을 실패로 보고했고 그중 6건이 검사기 결함이었다 -- 부정문 안의
`죄가 성립`, `강도상해죄` 안의 `상해죄`, `성립하며` 활용. 중복 앵커(같은 행위자·죄명이 상태만
달리 두 번)는 답안이 문언상 구별할 수 없어 그룹 집합 판정으로 바꿨다. **어휘 대조는 후보를
띄우는 도구이고 판정자가 아니다.**

### 다음

1. **P 조건.** `ANSWERPLAN_SPEC` 5.5 카드 회수. N은 동결됐으므로 P만 만들면 P−N이 선다.
   인용 규칙은 N의 판정(식별정보 보존)을 그대로 쓴다 -- 다른 자로 재면 차이가 섞인다.
2. **rubric judge.** 26문항 서브셋, 출력 경로 분리.

## 2026-08-14 P 조건 26문항 -- 실행 완료, N/P 대조 성립

**검수 문서는 `docs/analysis/v2_call3_p_26_run_review_ko.md`다.**

카드 회수(SPEC 5.5)를 구현해 P plan을 만들고 26문항을 실행했다. 프롬프트·모델·파라미터는
N과 동일하고 차이는 카드 하나뿐이다. 26/26, 오류 0.

- 회수: 417 검색 대상 -> 288 타깃에 카드 424개(고유 238장). tier ① 108 / ② 180 /
  ③ 근거없음 143(`UNMAPPED_DERIVED_ARTICLE` 49 + 인용문 부재 94).
- **계약 §4-10 신설**: 카드 주입이 issue·state·결론을 바꾸지 않는다. 단위 테스트가 두 plan을
  만들어 비교하고, 실데이터 26문항도 `[판례 법리]` 줄 424개에서만 다르다.
- 자동 판정: F4 0/105(양쪽), 식별자·인용·가담 전부 0. 상태 일치 N 101 / P 100이고 **잔여
  플래그는 양쪽 모두 sealed 문항에만 있다.** P는 불일치 0.

### 이 실행의 교훈 -- 지표가 조건에 편향돼 있었다

P의 첫 판정은 F4 5건·상태 6건 실패였고 전부 검사기 결함이었다. N은 plan 라벨을 복창했고
P는 법률 문장으로 바꿔 썼는데, 문자열 일치 검사가 **죄명을 말했는지가 아니라 라벨을 얼마나
베꼈는지**를 재고 있었다. 죄명 변형(`위계에 의한 ~`, `~등방화죄`, `·` 갈래), 문장 내 열거
두 형태, `기수범으로 처벌된다` 서술을 읽도록 고쳤다. 고치다가 공백을 지웠더니 `강간죄 및
상해죄`의 낱말 경계가 사라져 unmatched 27건이 났다 -- **유연해야 하는 것은 텍스트가 아니라
패턴이다.**

검사기 수정은 N에도 소급 적용했다. 답안 artifact는 불변이고 N의 completeness는 105/105로
동일, 상태 일치는 99 -> 101로 올랐다. 동결 대상은 답안이지 감사 도구가 아니다.
...
## 2026-08-16 · residual UNKNOWN 축

무슨 일이 있었나만 적는다. 지시는 `START_HERE.md`에 있다.

**결과: UNKNOWN 288 → 236 (48.4% → 42.1%), TRUE 286 → 299. 커밋 6건, 되돌린 실험 4건.**

### 진단을 먼저 하고 손댔다

288을 네 갈래로 갈랐다 — instance 전멸 46 / 주관적 요건 64 / 불능미수 34 / 나머지 144.
사례 본문은 한 번도 열지 않았다. Call 2 산출물에 rationale도 사건 텍스트도 없어서 열 필요가
없었고, 그래서 이 진단은 구조적으로 sealed-59를 열 수 없다.

이 분해가 없었으면 `START_HERE`의 옛 §5가 지목한 순서대로 갔을 텐데, 그 1순위 가설이
실측으로 기각됐다. `intent`의 결과적 가중범 갈래와 일반 갈래가 66.7%로 동일해서, "결과적
가중범 특유의 문제"가 아니었다.

### 무변경 재실행을 먼저 돌린 것이 이번 세션에서 제일 값진 결정이었다

첫 수정이 실패했을 때 손대지 않은 target 32건이 함께 움직였다. 그게 노이즈인지 변경 탓인지
모르면 이후 모든 A/B가 무의미해지므로, 변경 0으로 한 번 더 돌렸다. **595건 중 2건(0.3%).**
파이프라인은 사실상 결정론적이고, 그 뒤로는 모든 판정에 "손대지 않은 predicate 변동률"을
같이 실었다. 이 숫자가 없었으면 8.3% 오염을 낸 carrier 실험을 성공으로 읽었을 것이다.

### 레버는 셋이었고 조건이 갈렸다

* **증거 폭**(`evidence_scope`) — 사실이 *다른 행위에 서술된* 경우에만 듣는다. 취거·점유·
  고의·주거침입에서 들었고, 법적 평가(인과·귀속)나 서사에 없는 사실(흉기)에는 안 들었다
* **스케줄링** — 답할 수 없는 질문을 안 묻는 방향으로 두 번 다 먹혔다. 가로(앞 conjunct가
  UNKNOWN이면 멈춤)와 세로(막을 state가 성립한 뒤에 blocker를 물음). 부작용 1.4% 이하
* **프롬프트·payload** — 국소 수정이 아니다. shard 단위 요청이라 문구 하나가 같은 요청의
  다른 답까지 바꾼다. 최대 10.8%

### 실패한 것 넷을 근거와 함께 남겼다

가장 중요한 것은 carrier 실험이다. "신분 사실은 있는데 carrier가 못 본다"는 진단으로
`participant_status`·`related_party_action` 두 carrier를 설계·구현해 돌렸는데, carrier
텍스트는 실제로 넓어졌는데도(164→309자 등) 신분 predicate는 그대로였다. 진단이 틀렸다 —
그 사실은 **애초에 결박되지 않았다**. `gap.participant_status_not_bound`에 실패한 설계와
수치를 함께 적었다. 다음 사람이 같은 carrier를 다시 만들지 않게 하려는 것이다.

### 손대면 안 되는 것을 구분했다

instance 전멸 44건은 답하면 없는 범죄를 인정하는 것이라 개선 대상이 아니다. mental 자리
FALSE 0(100건)도 결함이 아니다 — 고의의 부정은 착오·책임 쪽으로 가도록 저작돼 있다.
UNKNOWN을 줄이는 것 자체를 목표로 삼으면 이 둘을 밟는다.

---

## 2026-08-16 · Call 3를 열고 배점의 28.7%에 입력을 넣었다

### 배점의 28.7%가 입력을 한 번도 받은 적이 없었다

KCL-26 rubric 735항목을 유형으로 집계하니 판례 법리 인용 15.6%, 죄수 8.2%, 학설 5.4%였다.
그 세 자리에 Call 3가 받는 payload는 **전량 0**이었다. 기계가 없어서가 아니었다 --
`build_v2_card_rule_statements.py`도, `--dispute-triggers`도, `build_v2_concurrence_condition_pairs.py`도
전부 만들어져 있었고 체인 스크립트가 인자를 넘기지 않았다. 배선만 하니 판례 법리 0→396,
학설 0→18이 됐다.

이 결함 클래스는 "단계가 존재하는가"를 보면 안 보이고 "그 단계의 산출물이 다음 단계 입력에
실제로 들어가는가"를 봐야 보인다. `concurrence_pairs`는 배선이 없어서 아무도 밟은 적이 없었고,
켜자마자 계보를 이어받지 않는 버그가 바로 나왔다.

### 죄수는 배선으로 안 됐다 -- 규칙이 2개다

pair 단계를 켜도 26문항에서 pair 1건, 그마저 absorption에서 UNKNOWN이었다. 원인은
`concurrence_rules.yaml`에 **저작된 흡수규칙이 2개**라는 것이다. 루브릭 죄수 60항목은
규칙베이스로 못 먹는다. 이 배점은 프롬프트의 자유 정리가 진다 -- 기계를 더 만들어도 안 된다.

### Call 3 계약을 A(닫힘)에서 B(부분 개방)로 바꿨다

A본은 세 문을 닫아 두고 있었고, 그 세 자리가 정확히 rubric이 배점하는 곳이었다 --
`analysis` 밖 죄명 금지(쟁점제기 14.6%), 죄수 임의확정 금지(8.2%), UNRESOLVED 유보 강제(결론 21.4%).
81%가 유보로 나가던 답안이 A본의 산물이다.

B는 결박한 것과 판단할 것을 나눈다. 성립·불성립은 확정, UNRESOLVED는 모델이 논증 후 결론까지,
`analysis` 밖 죄는 제기·검토까지만(최종 죄책 확정 금지), 죄수는 symbolic 우선.

**논문 주장을 지키는 선이 여기다.** 심볼릭이 자문에 불과하고 모델이 뒤집을 수 있으면 우리는
단계가 더 많은 CoT다. 자신 있는 것(결박된 결론, 요건 단위 법리·조문, 카드 판례)은 계속
강제하고, 못 보는 것(안 라우팅된 죄명, 죄수, 미결 요건의 마무리)만 넘겼다.

### 대가가 있었고, 표현 계약 보정으로 갚았다

B 1차에서 결론 누락이 0→4항목/3문항으로 늘었다. 전부 UNRESOLVED 앵커였다 -- 본문에서
논증해놓고 결론 문단에 안 올렸다. "성부는 본문의 논증에 따라 결론낸다"가 앵커로서
"빠뜨리지 말라"는 힘을 잃은 탓이다. 문구에 "결론 문단에도 반드시 올린다"를 더하니
1항목/1문항으로 줄었고, 남은 1건은 UNRESOLVED가 아니라 불성립 앵커였다.

같은 판에서 둘을 더 고쳤다. 카드 법리가 죄의 **정의**로 승격되던 것(제133조 제1항 판례를
제2항 죄의 구성요건으로 쓰는 doctrinal_error)과, 같은 행위자·죄명이 두 occurrence로 열릴 때
결론 목록에 상반된 상태로 두 번 실리던 것이다. 후자는 합치지 않고 사건 원문 인용으로 갈라
불렀다 -- 표지를 죄명 뒤가 아니라 줄 끝에 붙인 것은, 죄명 토큰을 건드리면 완결성 감사가
답안에서 그 죄를 못 찾아 멀쩡한 결론을 누락으로 신고하기 때문이다.

### 조문 오염은 품질 튜닝이 아니라 payload 오류였다

`legal_element.unlawful_appropriation_intent`가 절도죄 밑에서 저작돼 제329조를 달고 다니는데
같은 요건이 사기·횡령·강도에도 쓰인다. 사기죄 논증에서 절도죄 조문을 인용하게 만들고 있었고
26문항에서 각칙 조문 126건이 그랬다. 죄의 `statutory_refs`와 `derivation.base`까지를 범위로
잡고 총칙은 어디서나 허용하니 9건으로 줄었다 -- 남은 9건은 특수절도←제329조 같은 기초범죄
상속이라 정당하다.

카드 쪽에도 같은 종류가 있었다. 카드 코퍼스의 장(章)이 주석서를 따라와 제355조를 "횡령·배임"
하나로 묶고 있어서, 조문 키만으로 issue family를 잡으면 횡령 논증에 배임 판시가 붙는다.
항이 다른 죄를 레지스트리에서 계산해 회수 범위에서 빼고, **명제 필터는 tier와 무관하게** 걸었다 --
항으로 좁히지 못해 조문 전체로 되돌아간 target이야말로 형제 죄 명제가 들어올 자리다.

### 자동 지표를 네 번 틀렸다

죄명 커버리지를 `[가-힣]{2,12}죄` 정규식으로 뽑고 있었다. 숫자가 경계가 되어
`제3자뇌물교부죄`가 `자뇌물교부죄`로 잘렸고, `포괄일죄`·`기본범죄`·`후단범죄`가 죄명으로
잡혔고, 미수·교사를 죄명의 일부로 세어 `살인미수죄`가 미저작으로 계산됐다. 같은 질문에
26.7% / 27.2% / 52.2% / 39.3%를 차례로 답했다.

사용자가 출력에서 `기본범죄`·`후단범죄`·`위반죄`를 보고 지적해서 잡혔다. 추출을
`scripts/audit_v2_offense_coverage.py` 한 곳에 모으고 테스트 6개로 묶었지만, 잔여 조각이
남아 있고 sealed라 확인할 수 없다. **이 지표로 CoT와의 우열을 결론짓지 않기로 했다** --
사람이 직접 읽는 것이 이 단계의 판단 근거다.

### 채점을 쓰지 않았다

한 번뿐인 채점을 라우터 개선 전에 쓰면 낭비다. 이번 세션 API 호출 0건. 대신 무료로 얻을 수
있는 것을 다 만들어 뒀다 -- IDPR-B 동결본, CoT 26문항(8192), N 조건 plan.

### 이미 있는 자산을 감사하지 않고 두 번 만들었다

judge 입력 빌더를 새로 짜다가 지적받고 지웠다. `run_phase3_llm_judge.py`가 이미
`--methods-manifest`/`--case-id-file`/`--sealed-inventory`를 다 갖고 있고, 익명화와 셔플도 한다.
`build_v2_diagnostic_judge_inputs.py`가 `answer`→`generated_response` 변환까지 한다.
