...
공개 실행경로 정리 후 top-18 article 전체 범위를 첫 문항에 그대로 태운 Slurm 218171은
Call 2와 Scallop까지 완료했으나 Call 3에서 컨텍스트 한계를 확인했다. Call 2는 최초
81개, unknown 보강 45개, 관계 후속 59개를 단계적으로 판정해 최종 140개
issue status(37 satisfied / 2 not_satisfied / 101 unknown)를 만들었다. 그러나 작성 대상이
18개 죄명·105개 issue가 되면서 Call 3 입력이 최소 49,153토큰을 넘었고, 출력 상한
16,384토큰과 합쳐 Gemma 4의 65,536 컨텍스트를 초과했다.

원인은 카드 계층만이 아니라 KCL 설문의 task scope 손실이었다. 원문은 여러 번호의
사실관계를 공유하지만 일부 inventory row의 `question_prompt`에는 “사실관계 (1)” 같은
한정문구가 빠져 있었다. 또한 Call 1은 원문 전체 사실을 기록하면서 범위 밖 사실의
검색질의까지 만들었다. 첫 문항에서 (1) 성범죄를 묻는데도 (2) 사망과 (3) 뇌물 조문이
함께 선택된 것이 18개 죄명의 직접 원인이었다.

이를 다음처럼 교정했다.

- 별도 문단형 `사실관계 (n)`, 질문 선두형 `사실관계 (n)에서`, 본문 번호형 `(n)`,
  다중 범위 `(4)와 (5)`를 rubric 없이 원문 표지만으로 복원한다.
- 61문항 중 범위 표지가 해석되는 42문항은 지목된 사실 블록과 질문만 Call 1·1.5·2·3에
  일관되게 전달한다. 표지를 해석할 수 없으면 원문 전체로 안전 fallback한다.
- 10회 제1문 대표 검증에서 (1)은 협박·강요·강간미수·상해, (2)는 상해치사·폭행치사,
  (3)은 뇌물·전달금 임의소비·직무불이행 질의로 분리됐다.
- 범위 정규화 Call 1은 61행 중 58행을 새로 생성했다. 1행 출력 길이 종료와 2행 grounding
  실패는 직전 검증 graph를 재사용하고 `previous_valid_fact_graph` provenance와 실패
  사유를 남겼다. 실패 행을 누락하거나 조용히 대체하지 않는다.

Call 3도 추론값 복사를 줄였다. 모델은 `section_id`, `analysis_id`, `issue_status`와
provenance를 더 이상 복사하지 않고 IRAC 산문만 작성한다. 호스트가 issue 순서에 따라
이 값을 다시 붙인다. 첫 전체범위 요청의 동적 JSON Schema는 62,147자에서 11,961자로
줄었고, 모델이 생성해야 하는 내부 식별자도 제거됐다. 이 변경만으로 범위 밖 105개 issue를
정당화하지는 않으며, task scope 복원이 선행 해결이다.

Slurm 진입점은 spool 복사본의 `BASH_SOURCE`를 저장소 위치로 오인하지 않도록 전부
`IDPR_PROJECT_ROOT → SLURM_SUBMIT_DIR → PWD` 순으로 통일했다. Call 1·1.5·retrieval의
정식 산출물은 잡별 임시 파일이 완성된 뒤에만 원자 교체하며, 검색 모델은
`IDPR_DENSE_MODEL`과 `IDPR_RERANKER_MODEL`로 로컬 snapshot을 명시할 수 있다.

새 범위와 top-18 합집합을 사용한 1문항 E2E Slurm 218188은 5분 41초에 기술적으로
완주했다. Call 2는 51,386 prompt + 14,230 completion token으로 126개 issue를
19 satisfied / 1 not_satisfied / 106 unknown으로 판정했고, Call 3는 43,285 prompt +
9,650 completion token으로 답안을 생성했다. 컨텍스트 초과는 해소됐지만 20개 후보 조문
중 16개 죄명·93개 issue를 19,644자 답안으로 작성했다. 강간미수 사안에 특수강도,
준강도, 강도상해, 공갈, 공무집행방해, 방화 등이 포함되어 의미 품질은 실패다.

이 결과는 `unknown`이 거짓 양성을 막는 충분한 장치라는 초기 가정을 반증한다. 후보 조문이
최종 작성 범위로 그대로 전파되는 한, 해당 죄명에 고유한 사실이 없어 unknown이 된 것조차
긴 IRAC section을 만든다. 따라서 61문항 전체 Call 2를 바로 제출하지 않고 조문 라우팅의
정밀도와 검색의 재현율을 서로 다른 역할로 다시 분리한다.
