...
않는다. 이는 각 역할이 반드시 별개의 사람이라는 뜻이 아니다. 동일인이 여러 역할을
맡으면 같은 entity ID를 재사용한다. 사기죄 성립 rule에서는 피기망자와 처분행위자에
같은 변수를 사용하고, 그 사람과 재산소유자가 다른 삼각사기에서는 처분 권능 또는
지위를 별도 standard로 심사한다.
