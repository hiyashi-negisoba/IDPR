...
우리 쪽 원인이 라우터 miss인지 규칙베이스 공백인지는 별도 진단이 필요하다(CoT의 동일
누락은 우리 앞단 진단의 근거가 아니다).

**에피소드 ①도 양쪽 다 실패.** 8항목짜리 축인데 CoT는 강요죄/성폭법 촬영물이용강요로
갈음했고, 우리 답안은 **이 사실관계를 언급조차 하지 않는다**. Call 1.5가
`factual_episode:001`을 만든 것은 확인됐으므로, **episode 생성 이후
binding → realization → plan → AnswerPlan 중 어디서 탈락하는지**가 추적 대상이다.
어느 단계인지는 아직 특정하지 않았다.
...
1. **episode:001 소실** -- episode 생성은 확인됐다. `binding → realization → plan →
   AnswerPlan` 중 어느 단계에서 탈락하는지 추적한다. dev 케이스에서 8항목(배점 22%)이
   여기서 통째로 날아갔다.
...
4. **router recall** -- §4-3의 45건. 위 셋보다 뒤다. dev 케이스에서 주거침입이 안 열린 것이
   라우터 miss인지 규칙베이스 공백인지는 이 문서로 판별되지 않는다.
