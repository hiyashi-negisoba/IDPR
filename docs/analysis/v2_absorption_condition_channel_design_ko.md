...
## 카드 C 후속: 조건문에서 **권한 판단을 빼는 것**까지가 같은 원칙이다

> comment: pair 모델에게 "(1) 권한 없는 사용 (2) 그 인영의 위임장 현출" 두 가지를 보게 하면 안 된다.
> 권한 없는 사용은 다시 첫 offense의 element를 pair condition이 평가하는 셈이다. 이 채널이 물어야
> 하는 것은 하나면 된다 -- 흡수되는 쪽 instance에서 문제되는 인영이, 흡수하는 쪽 instance의 해당
> 문서에 현출되어 그 문서의 구성부분을 이루는가. 최종 해소 시점에는 어차피 첫 번째 instance가
> established라는 것이 이미 권한 없는 위조/부정사용을 보장한다. ref 이름은 내부 ID이므로 유지해도
> 되지만 `condition_statement`/`legal_standard`가 권한 유무를 다시 평가하게 만들지는 않아야 한다.

**반영.** 내가 바로 앞에서 세운 원칙("성립 여부는 elements가, 조건은 관계만")을 정작 조건문에서
지키지 못했다. 초안 문구의 "권한 없이 현출되거나 부정사용된 인영"은 조건이 두 개의 명제를 지게
만들었고, 그중 하나는 흡수되는 쪽 instance의 element였다.

```text
first_conduct에서 문제된 인영
  -> 그 인영이
  -> second_conduct의 문서에 현출되어 그 문서의 기명·날인 부분을 이루었는가
```

이것 하나만 남기고 저작을 다시 했다. `legal_standard`에는 "그 인영을 얻거나 사용할 권한이
있었는지는 여기서 판단하지 않는다"를 명시해, 모델이 빈칸을 스스로 메우지 않게 한다. ref 이름의
`unauthorized`는 내부 ID로 남긴다 -- 그 낱말은 payload에 실리지 않는다.

해소 시점의 논리곱이 그 근거다.

```text
seal_forgery_or_misuse established   <- 권한 없는 위조/부정사용은 여기서 이미 보장된다
+ private_document_forgery established
+ pair relation TRUE                 <- 이 채널이 지는 몫은 이 한 줄뿐이다
=> absorption
```

같은 원칙이 이번 채널까지 적용된다는 것이 여기서 확인된다: **각 neural target은 자기 몫의 atomic
proposition 하나만 판단한다.** 이 조건의 atomic proposition은 관계 하나다.
