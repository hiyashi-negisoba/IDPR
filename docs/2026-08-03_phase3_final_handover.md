...
| 2 | `218306` | 두 스모크 Call 1→1.5→L0→Call 2→Scallop→Call 3와 기존 출력 비교 | `experiments/results/phase3_final_design_e2e_v3` |
| 3 | `218307` | 봉인된 59문항 전체 생성 | `experiments/results/phase3_final_59` |

전체 회귀 `218301`은 `538 passed in 174.61s`로 통과했다.

첫 E2E 제출 `218302`는 모델 호출 전에 잘못 지정한 vLLM 0.17 환경이 Gemma 4를
인식하지 못해 실패했고 `218303`을 취소했다. 두 번째 `218304`는 vLLM 0.22에서 Call 1과
Call 1.5를 fallback 없이 통과했으나 L0의 로컬 Hugging Face 캐시 위치를 넘기지 않아
gated 원격 주소를 조회하며 중단됐고 `218305`를 취소했다. vLLM 0.22.0 환경과
`IDPR_HF_HOME=/data5/jaehoonjeong/.cache/huggingface`를 모두 고정한 최종 체인이
`218306→218307`이다.
...
```bash
squeue -j 218301,218306,218307 -o '%.18i %.20j %.2t %.10M %R'
sacct -j 218301,218306,218307 --format=JobID,JobName,State,Elapsed,ExitCode
```

성공 후 다음을 확인한다.

1. `phase3_final_design_e2e_v3/freeze_manifest.json`의 status가 passed인지 확인한다.
2. `phase3_final_design_e2e_v3/final_design_comparison.json`의 `passed`가 true인지 확인한다.
