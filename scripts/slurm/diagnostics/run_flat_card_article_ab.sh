#!/bin/bash
#SBATCH --job-name=idpr_call2_article_ab
#SBATCH --output=logs/idpr_call2_article_ab_%j.out
#SBATCH --error=logs/idpr_call2_article_ab_%j.err
...
RUN_DIR="$PROJECT_ROOT/.cache/call2_article_ab/${SLURM_JOB_ID}"
OUT="$PROJECT_ROOT/data/eval/card_status_smoke_article_ab_v2.json"
...
echo "=== IDPR call 2 article A/B start: $(date) ==="
...
"$CLIENT_PYTHON" scripts/diagnostics/run_flat_card_smoke.py \
    --base-url "http://127.0.0.1:${PORT}" \
    --model "$SERVED_MODEL" \
    --api-key "$LOCAL_API_KEY" \
    --work-dir "$RUN_DIR" \
    --out "$OUT" \
    --article-batches art298 art297 art301 art319

cleanup
echo "result=$OUT"
echo "=== IDPR call 2 article A/B end: $(date) ==="
