...
#     /data5/jaehoonjeong/IDPR/scripts/slurm/run_v2_absorption_condition_pairs.sh \
#     --execution-approved
#
...
RULES="${IDPR_CONCURRENCE_RULES:-$PROJECT_ROOT/data/v2/concurrence_rules.yaml}"
SERVED_MODEL="idpr-gemma-4-26b-a4b"
RUN_ROOT="${IDPR_ABSORPTION_RUN_ROOT:-$PROJECT_ROOT/experiments/v2_call15_directscope_26_causal/absorption_condition_v1}"
PAIR_PLAN="${IDPR_PAIR_PLAN:-$RUN_ROOT/pair_plan.jsonl}"
ARTIFACT="$RUN_ROOT/concurrence_condition_assessments.jsonl"
...
if [ "$(hostname -s)" != "$STATE_HOST" ]; then
    echo "absorption-condition client must run on service host $STATE_HOST; got $(hostname -s)" >&2
    exit 2
fi
...
echo "=== absorption condition pairs via existing service ${SERVICE_JOB_ID} ==="
echo "host=$(hostname -s) port=$PORT artifact=$ARTIFACT"

"$CLIENT_PYTHON" "$PROJECT_ROOT/scripts/run_v2_absorption_condition_pairs.py" \
