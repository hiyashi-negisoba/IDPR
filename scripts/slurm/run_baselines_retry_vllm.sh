#!/bin/bash
#SBATCH --job-name=baselines_retry_vllm
#SBATCH --output=logs/baselines_retry_vllm_%j.log
#SBATCH --error=logs/baselines_retry_vllm_%j.err
...
echo "=================================================================="
echo "🚀 Retrying 3 Failed Baselines (leprec, acal, legal_chain_reasoner)"
echo "Date: $(date -u)"
echo "Host: $(hostname)"
...
"$VLLM_BIN" serve "$MODEL_SNAPSHOT" \
    ...
    > "${PROJECT_ROOT}/logs/vllm_baselines_retry_server_${SLURM_JOB_ID}.log" 2>&1 &
...
# Baseline 4: LePREC (Unmodified Original) - retry after user_template kwarg fix
echo "▶ Running leprec..."
...
# Baseline 5: ACAL (Unmodified Original) - retry after user_template kwarg fix
echo "▶ Running acal..."
$PYTHON_BIN scripts/run_baselines_experiment.py \
    --baseline acal \
    --vllm-url "http://127.0.0.1:${PORT}" \
...
# Baseline 6: LegalChainReasoner (Unmodified Original) - retry after user_template kwarg fix
...
echo "3 retried baselines completed successfully!"
