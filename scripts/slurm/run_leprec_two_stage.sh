#!/bin/bash
#SBATCH --job-name=leprec_two_stage
#SBATCH --output=logs/leprec_two_stage_%j.log
#SBATCH --error=logs/leprec_two_stage_%j.err
...
echo "=================================================================="
echo "🚀 LePREC re-run as a two-stage baseline (issue decomposition -> zero-shot answer)"
echo "Date: $(date -u)"
echo "Host: $(hostname)"
...
"$VLLM_BIN" serve "$MODEL_SNAPSHOT" \
    ...
    > "${PROJECT_ROOT}/logs/vllm_leprec_two_stage_server_${SLURM_JOB_ID}.log" 2>&1 &
