#!/bin/bash
#SBATCH --job-name=ruleir_native_e2e
#SBATCH --output=logs/ruleir_native_e2e_%j.out
#SBATCH --error=logs/ruleir_native_e2e_%j.err
...
RUN_ROOT="${IDPR_NATIVE_E2E_ROOT:-$PROJECT_ROOT/experiments/results/rule_ir_native_kcl_e2e_${SLURM_JOB_ID}}"
...
echo "=== RuleIR-native KCL E2E start: $(date) ==="
echo "job=$SLURM_JOB_ID commit=$(git rev-parse HEAD) model=$MODEL_SNAPSHOT"
echo "run_root=$RUN_ROOT"
nvidia-smi --query-gpu=name,memory.total --format=csv,noheader

# The committed audit must still match every prompt before the model server starts.
"$CLIENT_PYTHON" scripts/audit_rule_ir_native_prompts.py
...
CASE_ARGS=()
if [ -n "${IDPR_NATIVE_E2E_CASES:-}" ]; then
    IFS=',' read -ra CASE_IDS <<< "$IDPR_NATIVE_E2E_CASES"
    for case_id in "${CASE_IDS[@]}"; do
        CASE_ARGS+=(--case-id "$case_id")
    done
fi

"$CLIENT_PYTHON" scripts/run_rule_ir_native_kcl_e2e.py \
...
echo "=== RuleIR-native KCL E2E end: $(date) ==="
