                        ...
                        auto.append(
                            {
                                ...
                                "action": dr.get(
                                    "note", "요건 제외 규칙 또는 보고문 → neural_query 불요"
                                ),
                            }
                        )
                        continue
...
def _write_guide(payload: dict[str, Any], queue: list[dict[str, Any]]) -> None:
    ...
    g.append(
        f"생성: 로컬 전용(`api_calls: 0`) · 대상: {payload['scope']} · "
        f"라우팅 근거: {payload['routing_source']}\n"
    )
    g.append("## 이 검토가 무엇인가\n")
    g.append(
        f"KCL 61문항이 커버하는 재산죄 17조문에서 NormCard **{c.get('card_deterministic_rule', 0) + c.get('card_standard_input', 0) + c.get('card_policy_variant', 0) + c.get('card_context_only', 0)}장**과 "
    ...
    g.append(
        "> 주의: 큐는 카드 목록이 아니라 **결정점 목록**이다. 지적이 없는 카드도 "
    ...
    g.append("|---|---:|---|")
    g.append(
        f"| 3.2 authority 라벨 정규화 | {c.get('authority_auto', 0)} | 이번 배치 포렌식: `rules/`·`rule_ir.schema` 0 hit → 결론 무관 |"
    )
    ...
    order = [
        ...
        (
            "3.1_variant_group",
            "경쟁견해 그룹",
            "판시사항 보고 그룹당 1회 선택 → 그룹 카드 일괄 정리",
        ),
        (
            "3.2_authority_upgrade",
            "판례 인덱스 대조",
            "출처가 실제 판례 인용 → 자동 상향 금지, 사람 대조",
        ),
        (
            "3.4_negative_query",
            "긍정형 질의문 승인",
            "초안 190건 작성완료 → confirm/수정만. 47건은 질의 불요",
        ),
    ]
    ...
    for t, meaning in [
        (
            "판례후보_조문일치_비공유",
            "참조조문이 해당 조문 + 경쟁카드와 비공유 → 방향 특정 가능성 최상",
        ),
        (
            "판례후보_조문일치_공유",
            "참조조문 일치하나 경쟁카드가 같은 판례를 공유 → 판시사항으로 가려야 함",
        ),
    ...
    for f, d in [
        ...
        (
            "property_negative_query_drafts.json",
            "3.4 긍정형 질의문 초안 (에이전트 작성 190건 + 질의 불요 47건)",
        ),
