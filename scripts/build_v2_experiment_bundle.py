...
def main() -> None:
    ...
    parser.add_argument(
        "--baseline",
        type=Path,
        default=ROOT / "experiments/results/baseline_unified_26/vanilla_zero_shot_outputs.jsonl",
    )
    ...
    conditions = {
        "N": {
            "answers": args.run_root / "call3_n_26/answers.jsonl",
            "plans": args.run_root / "answer_plan_v3_case_truths/answer_plans.jsonl",
            "answer_field": "answer",
            "description": "authored authority only; no card retrieval",
            "generation": {"model": "idpr-gemma-4-26b-a4b", "temperature": 0.7, "max_tokens": 8192, "samples": 1},
        },
        "P": {
            "answers": args.run_root / "call3_p_26/answers.jsonl",
    ...
    manifest["notes"] = [
        ...
        "Decoding parameters are identical across the three arms (0.7 / 8192).  The "
        "first baseline sweep decoded at 0.0 / 4096, so the baseline was re-run on the "
        "same live service rather than bundled as it stood; the original artifact at "
        "experiments/results/vanilla_zero_shot_outputs.jsonl is untouched.",
