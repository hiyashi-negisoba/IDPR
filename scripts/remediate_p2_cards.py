"""P2(비재산) 벌크 구조 정리 — `remediate_property_cards.py`를 그대로 이식.

P1 스크립트는 `.cache/.../rulegen_downstream/art*/`를 통째로 훑어 P1 산출물까지 다시
덮어쓰므로, P2 슬러그로만 제한하고 출력 경로를 분리했다(그 외 로직은 100% 동일).
...
OUT = PROJECT_ROOT / "data/rulegen/p2/remediated"
LEDGER = PROJECT_ROOT / "data/rulegen/p2/remediation_ledger.json"
...
def load_module_cards() -> dict[tuple[str, str], dict[str, Any]]:
    ...
    for p in DS.glob("art*/*/norm_cards/*.json"):
        ...
        if art not in P2_SLUGS:
            continue
        out[(art, p.stem)] = json.loads(p.read_text(encoding="utf-8"))
    return out
