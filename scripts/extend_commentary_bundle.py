...
SOURCE_PARQUET = Path(
    os.environ.get("IDPR_COMMENTARY_PARQUET", ROOT / "data/raw/commentary.parquet")
)
SOURCE_PDF_ROOT = Path(os.environ.get("IDPR_RAW_COMMENTARY_ROOT", ROOT / "data/raw/commentary"))
...
def main() -> None:
    ...
    for article_no, (title, issue_tag, rationale) in TARGETS.items():
        ...
        print(
            f"  {article_no} {title:14s} {len(chunks):3d} chunks / {len(batches)} 배치 "
            f"/ {chars:6,}자 / ${cost:.2f}"
        )
        ...
        json.dumps(
            {
                ...
                "correction": (
                    "'주석서 코퍼스 부재'라고 적었던 것은 오독이다. 원천 파싱본에 절 구조까지 "
                    "있고 원본 PDF도 있다. IDPR 번들이 KCL 태그 매핑에서 빠뜨린 것이다."
                ),
                "source": {
                    "parquet": str(SOURCE_PARQUET),
                    "pdf_root": str(SOURCE_PDF_ROOT),
                    "archive": os.environ.get("IDPR_COMMENTARY_ARCHIVE", ""),
                },
                "supplement_bundle": str(SUPPLEMENT.relative_to(ROOT)),
                "excluded": {
                    "제344조·제354조·제361조·제365조": "친족 준용 조문. 친족상도례는 처벌·소추 층이므로 절차 레이어(A4)에서 다룬다.",
                    "제336조·제339조·제340조": "KCL 61문항이 묻지 않는다.",
                },
                "unit_rates_usd_per_chunk": {
                    "extraction": USD_EXTRACT_PER_CHUNK,
