...
- actor 행위에 관한 기본 증거는 `evidence_occurrence.source_text` 하나다.
- `realization_context`가 있으면 같은 factual episode에서 provenance로 선택된 typed 보충 증거다.
  - `same_actor_action_evidence`만 target actor의 추가 행위로 귀속한다.
  - `context_evidence`는 객체·피해자·신분·관계·결과·상황을 이해하는 데만 사용하고,
    그 안의 다른 사람 행위를 target actor의 행위로 바꾸지 않는다.
  - `excluded_peer_actor_binding_ids`의 행위는 증거로 사용하거나 target actor에게 귀속하지 않는다.
- `question_assumptions`는 문항이 명시적으로 전제한 전역 사실만 담는 별도 typed
  carrier다. 비어 있지 않으면 해당 전제도 사용할 수 있지만, 새로운 actor 행위나 법률
  결론으로 확장하지 않는다.
- 위 typed carrier에서 직접 확인되거나 필연적으로 도출되는 사실만 사용한다.
- 문항의 다른 부분, 일반적인 사건 추측, 상식으로 보충한 사실은 사용하지 않는다.
- predicate의 definition-time `arguments`는 사건 값 binding이 아니다. actor는 fixed
  instance actor이고, 그 밖의 argument는 제공된 typed scope 안에서 존재적으로 판단한다.
...
- `semantic_exclusions`가 있으면 인접 개념과의 경계로 사용한다. 이는 사건 사실이 아니며,
  exclusion에 적힌 개념만으로 target truth를 만들지 않는다.
