...
- 호스트가 `predicates`를 전량 결정했다. predicate를 추가·삭제·병합하거나 카드별 질문으로
  확장하지 않는다.
- 사건 원문, 현재 issue에 선택된 `issue_facts`, 확정된 entity·role·relation, 해당 predicate에
  한정된 `authority_context`만 사용한다.
...
- 단순히 판례나 법리가 존재한다는 이유로 satisfied라 하지 않는다. 현재 사건에 적용된 사실평가를
  `reason`에 쓴다.
- `issue_facts`의 인식·착오·동의·목적 사실을 주관적 predicate 판단에서 빠뜨리지 않는다. 단순한
  범행 동기만으로 고의를 인정하지 말고 반대 방향의 인식 사실도 함께 평가한다.
