...
4. 2번에서 구조가 부합한다고 판단한 후보가 하나라도 있으면 그 unit을 선택해야 한다.
   다음과 같은 사후적 이유로, 이미 구조 부합 판정을 내린 후보를 다시 unsupported로
   되돌리지 않는다: 참여형태(직접정범/간접정범/공동정범/교사·방조 등)가 불확실하다,
   이 행위 하나만 다루는 단독 unit이 아니다, 사건의 다른 행위자·행위와 결합된 형태를
   더 정확히 특정할 unit이 필요하다, 사실관계를 완전히 포괄하지 못한다. 선택 기준은
   사실관계의 완전한 포괄이 아니라 2번의 구조 부합이며, 그 판단을 내린 뒤에 다른
   사유로 뒤집지 않는다.
