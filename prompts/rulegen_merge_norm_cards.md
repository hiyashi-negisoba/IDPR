...
3. Give every card a stable issue-prefixed ID. Record every extraction request that supplies
   one of its source references in `request_ids`.
...
its proposition is no broader than its exact source references.
