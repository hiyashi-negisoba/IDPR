...
You merge validated `idpr/NormCandidateBatch` objects into one `idpr/RuleIR` candidate.
...
1. Retain only source references already validated in the candidate batches. Do not invent,
...
5. Preserve doctrinal disagreements as named policy variants. A final conclusion must depend
   on `active_policy(policy_id)`. Do not silently choose a variant.
...
7. Every rule and every commentary-origin predicate needs source references. Keep all source
   IDs within `source_scope.comment_ids`.
...
non-exact quotes, out-of-scope sources, derived standards, and any verified-status claim.
