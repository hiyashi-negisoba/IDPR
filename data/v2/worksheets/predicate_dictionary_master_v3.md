# Predicate 사전 전체 통합 검수 게이트 — Master v3

[predicate_dictionary_master_v2.md](predicate_dictionary_master_v2.md)에 대한 사용자
검수 1건을 반영한다. v2의 "258은 supporting dependency" 판단은 맞았지만, 그 실제
predicate 저작을 Gate① 뒤의 2-pass 직전으로 미룬 것은 Gate①의 순서와 충돌했다.
v0·v1·v2는 그대로 둔다 — 이력 추적용.

---

## 정정 1 — 258 supporting dependency를 effective 1-pass에서 실제로 닫음

**v2 오류**: 258의2의 QUALIFY 대상 중 258(중상해·존속중상해) 연동 2갈래가 target
offense 정의에 필수임을 확정하면서도, 실제 predicate 저작은 "258의2 자신의 2-pass
조립 확정 전"으로 미뤘다. 이 상태에서는 Master inventory에 258이 미저작으로 남고,
Gate① 뒤 2-pass에서 새 1-pass 저작이 다시 열리게 된다.

**원칙 재적용**: 1-pass predicate 사전 → Gate① → 2-pass assembly 순서에서,
evaluation target 밖이라도 평가 대상 조문의 완전한 정의에 필수인 supporting dependency는
effective 1-pass 범위에 포함돼야 한다. 80개는 evaluation coverage 숫자이고, dependency
저작 범위와 같은 개념이 아니다.

```text
258의2 = ANY(group_or_multiple_force, dangerous_object_carriage) + base offense
base offense ∈ { 257 상해, 257 존속상해, 258 중상해, 258 존속중상해 }
```

이 4갈래 **전부가 258의2라는 target offense 자체의 정의**다. 그중 2갈래를 빼고
조립하면 "평가 대상 조문만 적재한다"가 아니라 **평가 대상 조문의 정의를 잘라
적재하는 것**이 된다. 게다가 이건 이미 이 프로젝트가 배치⑪(art319→322조)에서
확정해둔 원칙과 정면으로 충돌한다: **target list 밖의 조문이라도 target offense의
성립/완성 구조를 정의하는 데 필요하면 governing/dependency source를 따라간다**
(322조가 51개 목록 밖이어도 319의 CompletionPolicy 확정에 필요해 직접 열람했던
전례와 동일 논리).

**v3(완료)**: [`predicate_dictionary_ext_art258_v0.md`](predicate_dictionary_ext_art258_v0.md)에서
258을 최소 1-pass 저작했다. 257 base offense와 기존 존속 status/인식 predicate를
재사용하고, 제258조 제1·2항의 법정 중상해 결과를 나타내는
`legal_element.serious_injury_result` 하나만 신규로 확정했다. 258의2가 QUALIFY할
258(1)·(2) 중상해와 258(3) 존속중상해 두 variant가 이제 모두 실제 predicate vocabulary를
갖는다. 258은 evaluation/population에는 추가하지 않으므로 coverage는 **80** 그대로다.

---

## 최종 상태 (v3) — 전체 문서

이하 §1-§6은 v2 전체에 위 1건(258 최소 predicate 1-pass 완료)을 반영해 다시 수록한
것이다.
...
| 258의2 특수상해 | batch09(v0) | QUALIFY 4갈래 전부 유지, 258 dependency 최소 1-pass 저작 완료(§4-0) |
| 297 강간 | batch10(v1) | |
...
대상 아님으로 확정된 조문은 **36·328·344 세 건**. 258(중상해·존속중상해)은 80개
evaluation coverage에 포함되지 않지만 258의2 dependency로 필요한 최소 1-pass 저작을
완료했다(§1-C·§4-0).

### 1-C. 부수 커버리지 의존성
- **258(중상해·존속중상해)**은 population 대상(80개)이 아니지만, 258의2의 QUALIFY
  4갈래를 전부 유지하기 위한 supporting dependency로 최소 1-pass 저작을 완료했다
  (`predicate_dictionary_ext_art258_v0.md`). 신규 id는
  `legal_element.serious_injury_result` 1건이며, 258(1)·(2)와 258(3)의 두 base variant를
  2-pass에서 QUALIFY할 수 있다.
...
### 2.11 생명·신체 (250·254·255·257·259·263·267·268·258·258의2조)
...
| `legal_element.lineal_ascendant_of_self_or_spouse_status` | legal_element | 자기·배우자 직계존속 | 250 | 257, 258 |
| `legal_element.awareness_of_lineal_ascendant_status` | legal_element | 존속 인식 | 250 | 258(3) |
...
| `legal_element.serious_injury_result` | legal_element | 257의 상해로 생명위험이 발생했거나 불구 또는 불치·난치 질병에 이른 제258조상 중상해 결과 | 258(1)·(2), 직접 원문 | 258(1)·(2) 중상해 및 258(3) 존속중상해의 base result; 258의2 QUALIFY base 두 갈래 |
...
| `offense.aggravated_injury[258(1)-(2)]` / `offense.aggravated_ancestral_injury[258(3)]` | derived offense (2-pass assembly) | 257 상해에 `serious_injury_result`를 QUALIFY하고, 후자에는 기존 직계존속 status·인식도 QUALIFY | 258, `art258_v0` | 258의2 QUALIFY base 4갈래 중 2갈래; vocabulary는 Gate①에서 확정, 실제 YAML 조립은 2-pass |
...
**0건.** v3의 `legal_element.serious_injury_result`도 기존 인벤토리와 충돌하지 않는
신규 id 1건으로 확인됐다.
...
### 3-6. 커버리지 확대 감사에서 발견한 dependency 결손 1건 — 해소 완료
258의2(§2.11 참고)의 QUALIFY 4갈래 중 2갈래가 참조하는 258(중상해·존속중상해)이
population 목표(80개) 밖이라는 이유로 v1이 잘못 descope했던 것을 v2에서 발견했다.
v3는 이를 단순한 향후 저작 메모로 두지 않고 `art258_v0`에서 최소 1-pass로 실제
해소했다. art319→322조 선례와 동일하게, target list 밖 조문도 target offense 성립
구조에 필요하면 effective 1-pass source로 따라간다.

---

## 4. 통합 HOLD / 2-pass 확인 목록

### §4-0. 258의2 dependency — 258 최소 1-pass 저작 완료(해소)

258의2는 배치⑨에서 이미 QUALIFY 4갈래로 확정됐다:
...
이 4갈래 전부가 258의2라는 target offense 자체의 정의이므로, 그중 2갈래(258 연동)를
빼고 조립하면 target offense 정의를 잘라 적재하는 것이 된다. **해소**:
`predicate_dictionary_ext_art258_v0.md`가 258을 supporting dependency로 최소
1-pass 저작했다. 257을 base offense로 재사용하고, `legal_element.serious_injury_result`
하나로 제258조 제1·2항의 법정 중상해 결과를 확정했으며, 258(3)은 기존 직계존속
status·인식 predicate를 재사용한다. 따라서 **258의2 QUALIFY 4갈래는 전부 유지**되고,
그 네 base offense의 vocabulary는 모두 Gate① 전에 동결됐다. 258은 별도 평가 대상이
아니므로 80개 coverage에는 추가하지 않는다. 실제 YAML `DerivedOffenseDef` 조립만
Gate① 승인 뒤 2-pass에서 수행한다.
...
1. `docs/handoff/CURRENT.md`에 게이트 통과 기록 + 2-pass(`data/v2/definitions/`
   실제 조립) 시작점을 남긴다.
2. §4-0의 258 dependency 최소 1-pass 저작과 §4 (A)그룹 4건 전부가 확정된 상태로
   2-pass에 들어간다. 258은 evaluation population 밖으로 유지하되, 그 두 base variant의
   predicate vocabulary는 이미 Gate①에서 닫혔다.
...
### 6-1. 이번 게이트(v0→v3)에서 새로 재분류·폐기한 predicate
