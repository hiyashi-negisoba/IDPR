# Predicate 사전 전체 통합 검수 게이트 — Master v2

[predicate_dictionary_master_v1.md](predicate_dictionary_master_v1.md)에 대한 사용자
검수 1건(258의2→258 처리)을 반영한다. 나머지 v1의 6건은 전부 승인됐다. v0·v1은
그대로 둔다 — 이력 추적용.

---

## 정정 1 — 258 "descope"는 틀렸다: coverage에는 안 넣되 dependency로 최소 저작

**v1 오류**: 258의2의 QUALIFY 대상 중 258(중상해·존속중상해) 연동 2갈래를 "이번
게이트에서 descope"하고 257 갈래만으로 게이트를 통과시켰다.

**왜 틀렸는가**: 배치⑨가 이미 258의2를 다음과 같이 확정해뒀다 —
...
**v2(수정)**: §4-0을 다음 결정으로 교체한다 — 258은 eval/population coverage
(80개)에는 추가하지 않지만(별도 평가 대상이 아니라 258의2의 supporting dependency
이므로), **258의2를 완전하게 저작하기 위해 258(중상해·존속중상해)을 최소 저작**한다.
258의2의 QUALIFY 4갈래는 전부 유지한다. 실제 258 predicate 저작(카드/원문 열람
포함)은 이번 게이트(predicate ID freeze)의 범위 밖이고, **258의2 자신의 2-pass
조립을 확정하기 전에만** 완료하면 된다 — 다른 조문 2-pass의 전제조건 아님(art339
C-339와 동일한 "scoped precondition" 패턴). 80개 coverage 숫자는 그대로 유지한다.

---

## 최종 상태 (v2) — 전체 문서

이하 §1-§6은 v1 전체에 위 1건 정정을 반영해 다시 수록한 것이다.
...
| 258의2 특수상해 | batch09(v0) | QUALIFY 4갈래 전부 유지, 258은 dependency로 최소 저작 필요(§4-0) |
| 297 강간 | batch10(v1) | |
...
카운트에 포함되지 않으나 258의2 dependency로 별도 최소 저작이 필요하다(§4-0).

### 1-C. 부수 커버리지 의존성
- **258(중상해·존속중상해)**은 population 대상(80개)이 아니지만, 258의2의 QUALIFY
  4갈래를 전부 유지하려면 dependency로 최소 저작해야 한다 — 결정은 §4-0에서 확정,
  실제 저작(카드/원문 열람)은 258의2 자신의 2-pass 조립 확정 전에만 완료하면 된다.
...
| `legal_element.lineal_ascendant_of_self_or_spouse_status` | legal_element | 자기·배우자 직계존속 | 250 | 257, 258(저작 시 재사용 예정) |
...
| **258(중상해·존속중상해)** — **미저작, dependency로 최소 저작 필요(§4-0)** | — | 258의2 QUALIFY 4갈래 중 2갈래가 이 조문의 base offense predicate를 필요로 함 | — | 258의2 |
...
### 3-6. 커버리지 확대 감사에서 발견한 dependency 결손 1건
258의2(§2.11 참고)의 QUALIFY 4갈래 중 2갈래가 참조하는 258(중상해·존속중상해)이
population 목표(80개) 밖이라는 이유로 v1이 잘못 descope했던 것을 v2에서 정정
(§4-0). art319→322조 선례와 동일 원칙 — target list 밖 조문도 target offense
성립 구조에 필요하면 governing/dependency source로 따라가야 한다.

---

## 4. 통합 HOLD / 2-pass 확인 목록

### §4-0. 258의2 dependency — 258(중상해·존속중상해) 최소 저작 필요(결정 확정)
...
빼고 조립하면 target offense 정의를 잘라 적재하는 것이 된다. **결정**: 258
(중상해·존속중상해)은 eval/population coverage(80개)에 추가하지 않는다 — 별도
평가 대상이 아니라 258의2를 완전하게 저작하기 위한 supporting dependency이기
때문이다. 다만 **258의2의 QUALIFY 4갈래는 전부 유지**하고, 이를 위해 258을 최소
저작한다(카드/원문 열람 포함 실제 predicate 저작 — 319→322조 선례와 동일 원칙:
target list 밖 조문이라도 target offense의 성립/완성 구조에 필요하면 governing/
dependency source를 따라간다). 이 저작 자체는 이번 게이트(predicate ID freeze)의
범위 밖이고, **258의2 자신의 2-pass 조립을 확정하기 전에만** 완료하면 된다 — 다른
조문 2-pass의 전제조건 아님(art339 C-339와 동일한 scoped-precondition 패턴). 80개
coverage 숫자는 그대로 유지한다.
...
1. `docs/handoff/CURRENT.md`에 게이트 통과 기록 + 2-pass(`data/v2/definitions/`
   실제 조립) 시작점을 남긴다.
2. §4-0(258 dependency 최소 저작 결정)과 §4 (A)그룹 4건 전부 확정된 상태로
   2-pass에 들어간다. 258 자체의 실제 predicate 저작은 258의2 조립 확정 전까지
   별도로 완료한다(이번 게이트 범위 밖, 카드/원문 열람 필요).
...
### 6-1. 이번 게이트(v0→v2)에서 새로 재분류·폐기한 predicate
