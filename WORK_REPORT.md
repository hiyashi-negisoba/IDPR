...
## 재산죄 core 확정 + NormCardSet 조립 (2026-07-25)

KCL 커버 재산죄 벌크 HITL을 닫고, 검토 결과를 RuleIR이 입력으로 받을 수 있는 형식까지
끌어올렸다. API는 한 번도 쓰지 않았다.

### core 427장 확정

결정 A/A2(출처범위)·B(부정형 질의문)·C/C2(학설선택)를 카드에 일괄 반영한 뒤, 카드결함 감사와
절 계층 감사로 추가 강등하고 파서가 취지를 놓친 1건을 정정했다. 452 → **427장**
(standard_input 326 / deterministic_rule 101). 학설선택 승격 43장이 포함되고, 이중매매 21장은
별도 트랙으로 뺐다.

파서 오류 하나는 기록해 둘 만하다. "판례는 (1)인데 학계는 (2)를 강력주장함. 그냥그렇다고 우리는
판례를 따라가긴하니까.."를 `picked: ["1","2"]`로 읽어 다수의견과 반대의견을 **둘 다** 규칙으로
올렸고, 그 결과 매도인이 타인 사무처리자인지에 관해 서로 모순되는 두 규칙이 core에 남아 있었다.
취지대로 (1)만 규칙으로 두고 (2)는 학설 소개로 내렸다.

사기 core 88장도 같은 축으로 전수감사해 **84장**으로 정리하고(판단지침형·증명법 각 1 강등,
중복 2 병합, 메타 래퍼 2 재작성) RuleIR을 재빌드했다(강등 카드 단독근거 규칙 15·술어 8 제거).
`fraud_core_rule_selection_audit.json`의 `method: ..._no_api`는 "API 없이 수행"이라는 뜻이고
전수 인간검토를 의미하지 않는다 — 사기와 재산죄의 검토 수준이 다르다고 본 것은 오독이었다.

### NormCardSet 조립 — v7은 RuleIR 입력이 아니다

`property_core_set_final_v7.json`은 감사 원장이라 `source_refs`·`candidate_refs`·
`authority_basis`가 없다. RuleIR 생성기는 `idpr/NormCardSet` 계약을 요구하므로 merge 산출 카드에
v7 판정을 되돌려 반영했다(`scripts/build_property_core_norm_card_sets.py` → 조문별 16개 카드셋).

- `final_role` → `formalization`, 재작성 명제 47장, `review_required` 해제 277장,
  `disputed` → 확정 43장(판례 근거면 precedent_position, 그 외 settled). 전부 원장에 카드 단위 기록.
- **인용 축자성 복원 17건**이 실질 수정이다. 주석서 OCR이 낱말 중간에 넣은 공백을 모델이 정리한
  것 11건(`결 정되어야` → `결정되어야`)은 원문 구간으로 되돌리고, 문장을 압축한 6건은 정렬 구간의
  원문 문언으로 복원했다. 이걸 고치기 전에는 제355조·제328조가 계약 위반이었다.
- merge 단계 계약까지 건 엄격검증에서 47장/427에 지적이 남는다. 32장은 후보 polarity=negative를
  카드가 exception으로 재분류한 것으로 정상 승격이고, 나머지는 추적성 드리프트다(인용 출처가 연결
  후보 밖 10·norm_kind 혼합병합 8·후보 id 미존재 6). 명제 자체는 HITL에서 사람이 읽은 문장이므로
  RuleIR을 막지는 않으나 원장에 남겼다.

### 결정 B-2 — 예외형 카드가 질의문 심사를 건너뛰었다

`rebuild_decision_b.py:70`이 `polarity == "negative"`만 큐에 담았다. 그런데 merge가 부정형 명제
중 구성요건 배제 사유를 `exception`으로 재분류했기 때문에, 그 카드들은 부정형 명제를 그대로 들고
심사를 통과해 버렸다. core의 negative/exception 127장 중 **44장이 승인된 질의문 없이 남아 있었다.**

standard_input 32장은 긍정형 질의문을 작성하고, deterministic_rule 12장은 질의 면제로 사유를
남겼다(`결정B2_예외형질의문승인.md`). 부정형 명제가 모델에 도달하지 않게 하는 장치이므로 승인
전에는 배선하지 않는다.

### 검증

- 조문 16개 전부 `validate_norm_card_set` 계약 통과(core 427장).
- 초안 스크립트에 게이트를 걸어 두었다 — 초안 대상 집합이 실제 누락 집합과 다르거나
  standard_input/deterministic_rule 구분이 어긋나면 `SystemExit`.
- `pytest -k "rulegen or norm_card"`: 37 passed.

### 다음 게이트

1. 결정 B-2 승인(질의문 32 + 면제 12) → 승인분 배선.
2. **RuleIR 생성 단위 결정** — 카드셋은 조문별인데 RuleIR은 죄명 단위여야 한다(검증기가 `issue_tag`
   일치를 요구하고 라우터가 쟁점 태그로 규칙집합을 고른다). 제355조는 횡령/배임 분할이 필요하다.
3. preflight 10항목 승인 → RuleIR 생성·비평(~$3, 예산 게이트).

상세: `docs/research/idpr_remaining_work.md` A3.

## 결정 B-2 반영 + RuleIR 죄명 단위 확정 (2026-07-25)

### B-2 검토 반영 — core 422장

1부 32건 중 11건에 코멘트가 달렸고 2부 면제 12건은 전부 승인됐다. 코멘트는 승인할 문안이 아니라
해석 입력이므로 취지를 읽어 반영했다(`scripts/apply_decision_b2.py`, 원장 `결정B2_반영원장.json`).

- **강등 5장**: 사후 반환의사("있어도 유죄라는 취지인데 굳이 꼬을 필요 없다"), 범행 전 상계정산
  ("특별한 사정이 애매하다"), 수분양권 위임(일반 규칙과 중복 → 일반 규칙을 남김), 법인 운영권
  양도 청탁(지엽), 다른 원인에 의한 교부(같은 조건을 묻는 카드와 근사중복 병합).
- **질의문 수정 3건**: 명의신탁은 "대외적 소유명의" → "법률상 소유자", 권리행사는 원문에 없던
  요건(채권액 비교)을 빼고 "고지한 해악의 내용이 권리행사였다"로, 개별 위임요소는 원문에 없는
  "약정"을 빼고 "관리·처리에 관한 요소가 있었다"로.
- **질의 면제 이동 1장**: 행정절차상 불법 카드는 면허 미취득을 이유로 업무성을 부정하는 항변을
  차단하는 규칙이다. 업무성을 가르는 사실은 같은 절의 "사무 내용 자체의 위법성"이 묻고 있으므로
  모델에 물을 사실이 없다 → deterministic_rule.
- **확인만 한 2건**: 제350조 경계 카드의 극성 방향은 초안대로 맞다. 혼동은 이 카드가 공갈죄
  배제(exception)이면서 문언으로는 강도죄 성립을 긍정해 "카드 발동"이 모호했던 데서 왔다 — 이후
  문서에는 발동 시 결론을 함께 적는다. 배임증재 대칭은 이미 확보돼 있다
  (`art357_sec4.giver_view_justification`이 증재자 관점, 이 카드가 수재자 관점).

core 427 → **422장**(standard_input 320 / deterministic_rule 102), 질의문 26 + 면제 13.

### RuleIR 단위 = 죄명 9 + 공유 모듈 2

조문을 단위로 잡을 수 없다. 검증기가 RuleIR과 NormCardSet의 `issue_tag` 일치를 요구하므로
(`src/idpr/rulegen/__init__.py:701`) 제355조 하나를 단위로 두면 라우터가 "횡령 쟁점"으로 부른
규칙집합 안에서 배임 요건이 함께 발화한다.

분할 신호는 데이터에 있었다. 제355조는 `comment_id`에 항이 박혀 있어 **1항 60장 / 2항 33장,
혼합 0장**으로 기계 분할된다. 제356조는 항 표시가 없고 절 구조로 갈리는데, 업무 개념 카드 10장은
업무상횡령·업무상배임이 똑같이 쓰므로 어느 한쪽에 넣을 수 없다.

`scripts/build_rule_ir_units.py` → 단위 11개(`data/rulegen/property/rule_ir_units/`), 422장,
전 단위 계약 통과. 가중유형은 기본죄와 한 단위에 뒀다(특수절도→절도, 강도류→강도) — 별도
규칙집합으로 쪼개면 절취·폭행 같은 기본 요건 카드를 복제해야 하고 두 벌이 갈라진다.

### 친족상도례 — 성립이 아니라 처벌·소추 층

주석서 카드가 직접 말한다: "제328조 제1항의 형 면제는 범죄 성립에는 영향이 없고 형벌만 면제하는
인적 처벌조각사유이다." 2항은 친고죄다. 준용 범위도 죄명마다 다르다(제344·354·361조, 손괴 제외).

그래서 죄명 규칙집합마다 복제하지 않고 **독립 규칙집합 + 브리지 술어**로 받는다. 죄명 규칙집합이
`property_crime_established(case_id, crime_id, defendant_id, owner_id, possessor_id)`를 배출하고,
친족 규칙집합이 그것과 신분관계 사실만 입력으로 받아 `punishment_exempt` /
`prosecution_requires_complaint`를 판정한다. 준용 범위는 `kinship_applicable_crime(crime_id)`
화이트리스트 한 곳에서 관리한다. 이 구조여야 헌재 2024.6.27. 결정(1항 적용중지)이나 장애인복지법
연계(2022.1.28 시행) 같은 시행일 조건을 한 곳에만 넣을 수 있고, "죄는 되지만 처벌되지 않는다"를
IRAC 결론에서 무죄와 구별해 쓸 수 있다. 같은 구조를 업무자 가중신분(제356조 공유 10장)에도 쓴다.

### 강도 기본조문 — 자산 부재가 아니라 선별 누락이었다 (정정)

`robbery` 단위에 강도 기본 구성요건이 없는 것을 처음에 "주석서 코퍼스 부재 → uncovered"로 적었다.
**오독이다.** 사용자 지시로 `sp` 디렉토리를 찾아보니 둘 다 있다 — 원본 PDF
(`sp/commentary_criminal.zip → casenote_pdfs/044_제333조 [강도].pdf`, casenote 373개)와 절 구조까지
파싱된 chunk(`sp_qwen/data/serve/commentary_chunks/docs.parquet`, 형법 4,011 chunks / 251 조문,
제333조 51 chunks — Ⅱ.3 피해자의 반항억압 6 chunks가 핵심). IDPR 번들(3,108/96)이 KCL 태그 매핑에서
빠뜨린 것이었다.

`law_id`를 봐야 한다는 점도 기록해 둔다. 001692가 형법, 001671이 형사소송법이고, 형법 제342조
(미수범)와 형소법 제342조(일부상소)가 같은 조번호를 쓴다 — 조번호만으로 대조하면 섞인다.
형법 제342·344조는 절 파싱본이 없어 번들이 원천 PDF 전문(raw_pdf 스텁)으로 보완했고, 이는
`kcl_criminal_v1_non_mapped_audit.md`에 이미 기록돼 있던 처리다.

기존 카드가 전제하는 조문만 보강 대상으로 추렸다(`scripts/extend_commentary_bundle.py`).

| 조문 | chunks | 배치 | 추정 | 근거 |
|---|---:|---:|---:|---|
| 제333조 강도 | 51 | 4 | $2.60 | robbery 단위 기본 구성요건 |
| 제332조 상습범 | 15 | 2 | $0.76 | `art329_sec8_3`이 전제 |
| 제330조 야간주거침입절도 | 5 | 1 | $0.25 | `art328_sec4_1`·`art329_sec8_3`이 전제 |
| | **71** | **7** | **$3.61** | |

정본 번들은 덮어쓰지 않고 보강 번들을 따로 냈다(기존 캠페인 산출물 재현성 유지). 제336·339·340조는
KCL 문항이 묻지 않아 제외, 친족 준용 조문은 절차 트랙과 함께.

### 친족상도례는 절차 레이어로 (사용자 결정)

배선을 A4로 넘긴다. 형 면제(1항)·친고죄(2항)는 성립이 아니라 처벌·소추 층이고, 소추조건은 고소의
존재·기간·취소라는 절차 쟁점 그대로다. 재산죄 트랙에서는 카드를 독립 모듈로 떼어 두는 데까지만
하고(25장, 계약 통과), 브리지 술어를 preflight에 넣어 두고 받는 쪽 규칙은 절차 레이어에서 쓴다.

### 다음 게이트

1. **보강 추출 승인**(제333·332·330조, $3.61) → core 판정 후 `robbery`·`theft` 단위 합류.
2. preflight 10항목 승인 → **RuleIR 생성·비평**(~$3.3). 단가 근거는 사기 실측 1회 =
   terra $0.17 + sol $0.43 = $0.60/88장.

둘 다 API 호출이므로 **sbatch로 제출**한다. 순서를 지켜야 한다 — 강도 기본 요건 카드가 들어온 뒤
`robbery` RuleIR을 만들어야 규칙집합을 두 번 만들지 않는다.

상세: `docs/research/rulegen_rule_ir_units.md`

## 보강 3조문 추출 완주 + 전수 판독 (2026-07-25, job 213426)

강도 기본조문 등 3조문을 sbatch로 보강 추출하고(9분 17초), 나온 145장을 재산죄 벌크와 같은 축으로
전수 판독해 검토 요청까지 만들었다.

### 추출 실적

| 조문 | chunks | 후보 | NormCard | 모듈 | 추출 | 다운스트림 |
|---|---:|---:|---:|---:|---:|---:|
| 제330조 야간주거침입절도 | 5 | 17 | 17 | 5 | $0.15 | $0.51 |
| 제332조 상습범 | 15 | 34 | 34 | 9 | $0.19 | $0.85 |
| 제333조 강도 | 51 | 94 | 94 | 28 | $0.49 | $2.68 |
| | **71** | **145** | **145** | **42** | **$0.83** | **$4.04** |

critic verdict revise 38 / pass 4(첫-패스 정상). **실지출 $4.87로 견적 $3.61을 35% 넘겼다.**
원인은 견적 방식이다. 재산죄 벌크 평균 $0.0367/chunk를 그대로 썼는데 다운스트림 비용은 모듈 수에
붙는 고정비(스키마 + gold exemplar 프롬프트)가 지배한다 — 제330조가 5 chunks에 5 모듈로
$0.102/chunk, 제333조는 51 chunks에 28 모듈로 $0.0525/chunk다. **소규모 조문 견적은 chunk가 아니라
모듈 수 기준으로 잡아야 한다.** 누적 $62.4 / $100.

### 전수 판독 — 145장 → core 49장

`scripts/audit_supplement_core.py`. 자동 표지 regex를 돌리지 않고 145장을 직접 읽었다. 재산죄에서
표지 재현율이 48%였고, 이 규모에서는 전수 판독이 더 싸다. 조문 성격상 두 가지를 조정했다.

- **제332조는 '상습범'이 총칙 표지에 걸리지만 구성요건이다.** 조문 자체가 가중유형을 정하므로
  자동 표지를 그대로 돌리면 조문 전체가 날아간다. 반대로 특가법·성폭력처벌법·특정강력범죄법
  조문은 타법률 축으로 내렸다(7장).
- **제333조는 94장 중 33장이 Ⅹ(죄수)·Ⅸ(공범) 절이다.** 축대로 내리되, KCL 문항이 실제로 묻는
  쟁점(현금카드 강취 후 인출, 상습절도-주거침입 관계, 제334조 경계)은 강등하지 않고 확인 항목으로
  올렸다.
...
| 증명·소송법(P) | 3 |
| 근사중복 | 1 |
| merge 판정(context_only) | 1 |

근사중복 1장은 제333조 채권자 살해 카드다. 제338조 `debt_evasion_no_robbery`가 같은 규칙이고
결정B에서 질의문 승인까지 받았으므로 그쪽을 남겼다.

core 잔존 **49장**(art330 4 · art332 9 · art333 36), 그중 부정형·예외형 15장. 학설선택 23장과
확인 5건은 사용자 검토 대기다. 49장의 인용 58건은 전부 축자 일치라 이번에는 복원이 필요 없었다.

### 다음

`보강3조문_검토요청.md` 승인 → 학설선택·확인 반영 → core 카드셋 조립 → `theft`(50+13)·
`robbery`(56+36) 단위 재빌드 → preflight → RuleIR(sbatch).

## 보강 3조문 검토 반영 + 부정형 질의문 (2026-07-25)

### 학설선택 10그룹 — 2건은 내 제안이 틀렸다

사용자 검토에서 8건은 제안대로, **2건은 정정을 받았다**(`scripts/apply_supplement_review.py`).

- **제333조 선행행위로 조성된 상태의 이용** — 판례는 `unconsciousness_prior_force_no_causation`이다.
  나는 반대 견해(자기 선행행위로 조성한 상태의 이용 = 강취)를 제안했다. 판례는 탈취 목적 없는
  선행행위와 탈취 사이의 인과관계를 부정하고, 살해 후 취거는 살인죄와 절도죄의 경합으로 본다.
  그룹 7(강간 폭행 후 탈취 = 강도, 판례)과 갈리는 지점은 **피해자의 의식 유무**다 — 공포 상태의
  피해자로부터의 강취는 강도지만 심신상실 상태에서는 억압할 반항 자체가 없어 절도다. 두 결정이
  이 경계에서 정합한다.
- **제333조 권리행사와 강도** — 대법원은 소극설(`right_exercise_robbery_negative`)이다. 나는
  적극설을 제안하면서 "공갈죄 쪽 core 카드와 층이 맞는다"를 근거로 들었는데, 그 근거가 실은
  반대 결론을 가리켰다. 권리행사면 위법성이 조각된다는 제350조 카드와 정합하는 것은 소극설이다.

승격 10장(전부 학설 어법 제거 재작성) / 학설 강등 13장. 2부 확인 5건은 무응답이라 감사 판정
(죄수·구체사안 축 강등)을 적용했고, 되돌리려면 `USER_CHECK_DECISION` 한 줄만 바꾸면 된다.

### core 481장 · 단위 11개 재빌드

v8 422 + 보강 59 = **481장**(art330 7 · art332 9 · art333 43). 조문별 카드셋 19개, 죄명 단위 11개
모두 계약 통과. 강도 기본조문이 들어오면서 `robbery`가 56 → **99장**이 됐고 반항억압·강취·
불법영득의사·착수·기수라는 기본 구성요건이 규칙으로 채워졌다. `theft`는 야간주거침입절도·상습절도가
합류해 50 → **66장**이다.

### 결정 B-3 — 질의문 13건 + 면제 6건

core 481장 중 부정형·예외형이면서 승인 질의문이 없는 19장을 처리했다
(`결정B3_보강조문_질의문승인.md`). B-2에서 받은 지적을 반영해 **발동 시 결론**을 항목마다 적었다 —
"카드 발동"이 공갈죄 배제인지 강도죄 성립인지 모호했던 문제다.

방향을 뒤집은 것이 4건이다(`not_satisfied`). '반복만으로는 부족하다'나 '객관적으로 이르지
못한 경우'를 그대로 물으면 이중부정이 되므로, 습벽 발현 사정이나 폭행의 객관적 정도처럼 **있는
쪽 사실**을 묻고 발동 방향을 반대로 적었다.

### 다음

B-3 승인 → 질의문 배선 → preflight 10항목 → RuleIR 생성·비평(~$3.5, sbatch).

## 결정 B-3 반영 — 질의문 3건 수정 + 카드 병합 (2026-07-25)

지적 4건 중 3건은 내가 원문을 잘못 읽은 것이었다(`scripts/apply_decision_b3.py`).

**제332조 동종성** — "절도·강도·사기는 죄종을 번갈아 한 게 아니라 저 세 개가 상습 가중조항을 가져
포괄하여 설명한 것"이라는 지적. 주석서는 "상습성은 동종의 형태의 행위를 반복누행하는 습벽을 말하는
것이므로 **예컨대** 서로 유형을 달리하는 절도, 강도 및 사기 등을 반복한 경우에 절도의 상습성을 인정할
수 없다"고 쓰고 바로 다음 문장에서 "그러나 단순절도·야간주거침입절도·특수절도는 동종의 절도행위이므로
포괄하여 하나의 상습범"으로 이어진다. 이 카드가 담는 규칙은 **동종성 판단의 범위**다. 초안 질의문은
강도 전과가 섞이면 절도 상습성이 부정되는 것처럼 읽혀 과잉이었다 — 실제로는 그 경력을 상습성 근거에서
제외할 뿐이다. 명제와 질의문을 다시 쓰고 발동 시 결론을 "근거에서 제외"로 정확히 적었다.

**제333조 강도·공갈 경계** — "반항억압 강제탈취가 있고 그보다 조금 더 약한 수준이라는 점이 질의문에
안 나타난다". 정도의 상한이 빠져 있었다. "반항할 수 있는 상태에서 스스로 결정하여 건넸다"로 상한을
긍정형으로 넣어 강도 쪽 core 카드(객관적 반항억압)와 상호배타적으로 걸리게 했다.

**제333조 부동산 재물성** — "이것도 좀 이상해". 원문이 두 문장 한 세트다: "부동산은 도취죄 객체인
재물인지 견해가 대립되나 소극적으로 봄이 타당" + "폭행·협박으로 부동산에 관한 권리를 강취한 경우
재산상 이익에 대한 강취로 보면 족하다". 카드를 쪼개 놓으니 "재물 아님"만 발동하고 그 다음이 끊겼다.
**병합**했고 질의문도 객체를 한 번에 묻는다("강취의 대상은 부동산 또는 부동산에 관한 권리다").
학설선택에서 둘 다 살리기로 한 것이 실은 하나의 규칙이라는 뜻이었다.

병합을 위해 조립기에 `absorb_source_refs_from`을 넣었다 — 흡수된 카드의 인용·후보를 남는 카드의
근거로 합친다. 명제가 두 문장을 담으므로 인용도 둘 다 있어야 축자성이 성립한다.

**core 480장**(v9 481 → 병합 1), 조문별 19개·단위 11개 전부 계약 통과.

### 가중유형 on/off (설계 요청 반영)

"가중적 구성요건을 기본범과 구별해서 on/off 하게 하면 좋겠다"는 요청은 질의문 문제가 아니라 RuleIR
출력 술어 설계라 `rulegen_rule_ir_units.md` §3.1로 세웠다. 출력을 3층으로 나눈다 —
`<crime>_established`(기본범) / `<crime>_aggravation(kind)`(가중 플래그, 열거) /
`charge(label)`(죄명 확정). 기본 요건 카드를 복제하지 않고, 플래그가 꺼지면 자동으로 기본범으로
내려가며, 결과적 가중범(치상·치사)은 폭행 고의 카드를 플래그의 전제조건으로 요구한다.
preflight 정식 항목으로 넣었다.

## RuleIR preflight 10항목 + 레벨 구조 정리 (2026-07-26)

### preflight 10항목 (승인 대기)

사기 RuleIR이 통과했던 것과 같은 형식의 게이트를 재산죄용으로 만들었다
(`scripts/build_property_rule_ir_preflight.py` → `RuleIR_preflight_10항목.md`).
scope / unit_granularity / outputs / **aggravation** / actor_roles / **bridge_predicate** /
neural_state / evidence_gate / fewshot / api_ceiling.

사기와 달라지는 항목이 셋이다. ① 단위가 11개라 단위별 출력 술어가 다르다. ② 가중유형을 기본범과
분리해 플래그로 판정한다(사용자 요청). ③ 공유 모듈이 브리지 술어로 붙고, 친족상도례는 **배출하는
쪽까지만** 만든다(받는 쪽은 A4).

행위자 역할에서 강도·공갈은 슬롯을 늘렸다. core 카드
`art333_sec2_4.target_person_obstructing_taking`(폭행의 상대방은 탈취에 장애가 되는 자이면 된다)이
폭행 상대방과 소유자·점유자의 분리를 요구하고, 공갈은 외포된 자와 처분한 자가 갈린다.

부정형 카드의 질의문 보유를 게이트에서 확인했다 — 승인 116 / 면제 26 / **미보유 0**.

### 레벨(페이즈) 구조 — 카드로 확인했다

`docs/research/rulegen_rule_ir_phases.md`, 기계 판독본 `rule_ir_phase_map.json`.
추측으로 그리지 않고 core 480장을 L0~L7에 배정해 각 레벨이 무엇으로 채워지는지 봤다.

L0 적격·객체 123 / L1 실행행위 120 / L2 인과 3 / L3 주관 70 / L4 단계 30 / L5 가중 99 /
L6 위법성 10 / L7 처벌·소추 25.

레벨 순서는 계약과 맞물린다. 부정은 `case_assessment_complete` 이후 최종 스트라텀에서만 허용되므로
위법성 조각(L6)을 요건 층에 둘 수 없다 → `elements_met`(L0~L4, 부정 없음) →
`established`(최종 스트라텀에서 `not justified`) → `aggravation`(L5) → `charge` 순서가 강제된다.

죄명별 무게중심이 그대로 드러난다. 절도는 L0가 40장 중 23장(다툼이 "누구의 점유였나"에 걸린다),
강도는 L5가 56장으로 기본조문 42장보다 크고(준강도 26장), 횡령은 L3가 26장(불법영득의사가 승부처),
권리행사방해는 L0가 23/32장이다.

**빈 레벨은 coverage 보고 항목이다.** L2가 480장 중 3장뿐인 것이 가장 크다 — 재산죄 주석서는
인과관계를 독립 절로 두지 않고, 일반론은 형법총칙 주석서가 필요한데 코퍼스에 없다(KCL 태그
`causation`·`objective_attribution`이 이미 uncovered). 점유이탈물횡령 L3, 권리행사방해 L4,
공갈 L5(특수공갈 미커버)도 빈다.

배정은 **주석서 절 제목을 1차 신호로** 썼다. 명제 문언만으로 하면 480장 중 166장이 L0로 쏠린다 —
객체 명사가 행위 카드에도 들어 있기 때문이다. 절 제목이 명제보다 강한 신호라는 것은 카드결함 감사에서
이미 확인됐던 것이고 이번이 두 번째다. 기수·미수를 직접 정하는 명제는 절 제목보다 우선시켰다
(주석서가 기수시기 규칙을 '행위' 절 안에 적어 두는 경우가 있다).

## RuleIR 생성 2회 실패 — 사기 선례를 잘못 읽었다 (2026-07-26)

preflight 10항목 승인 후 RuleIR 생성을 두 번 제출했고 **둘 다 10단위 전부 계약 검증에서 막혔다**
(job 213797 $0.14, job 213799 $0.17, 합 $0.31). 1차 실패는 고칠 수 있는 것이었지만 2차 실패가
전제 자체의 오류를 드러냈다.

### 1차 (job 213797) — 검증기가 사기 전용이었다

`validate_full_rule_ir_generation`이 식별자를 문자열로 박아 두고 있었다: 출력 술어
`fraud_established`·`fraud_not_established`, 시스템 입력 `fraud_case_roles`(기망자·처분자 슬롯),
최종 부정 규칙 id `fraud.core.outcome.established`, 허용 부정 집합
`{fraud_has_negative, fraud_has_conflict}`. 절도 RuleIR이 `theft_established`를 만들면
"missing required output predicate **fraud**_established"가 뜬다. 사기 전용 검증기를 다른 죄명에
그대로 걸었던 것이 잘못이다.

`RuleIRGenerationProfile`로 파라미터화하고 `for_crime(issue_tag, actor_roles)`를 뒀다. 기본값이
사기 그대로여서 사기 계약 동작은 불변이고 테스트 150 passed. 이 변경이 사기 preflight의 artifact
게이트("검토 후 변경")를 정확히 발화시켰으므로 사유를 `artifact_rehash_log`에 남기고 재해시했다.

내 레벨 설계도 계약을 위반하고 있었다. `established :- ..., not justified`로 적었는데 계약은
**부정을 쓰는 규칙이 단 하나**이고 정확히 `has_negative`·`has_conflict`만 부정해야 한다. 즉 L6
위법성 카드는 `justified`가 아니라 `has_negative`로 흘러야 한다. 요청에 그 구성과 시스템 입력 4개,
hard requirement 10개를 명시했다.

### 2차 (job 213799) — 생성 자체가 성립하지 않는다

6단위가 **빈 출력**(규칙 0·술어 0, 최상위 필드 누락)이고, 출력이 나온 4단위도 부분이었다 —
권리행사방해는 승인 카드 32장 중 25장을 누락했고, 점유이탈물횡령은 규칙 5개·술어 23개에 그쳤다.

원인을 찾으려 사기 선례를 열어 보니 **사기 RuleIR은 terra 1콜로 만든 것이 아니었다.**

| | 규칙 | 술어 |
|---|---:|---:|
| 사기 terra 산출물(`.cache/.../terra/fraud.article347.rule_ir.full.v1.json`) | **4** | **6** |
| 사기 최종 후보(`fraud_full_rule_ir_candidate_unreviewed.json`) | 349 | 202 |

349 규칙은 `scripts/build_fraud_full_rule_ir_candidate.py`(2,377줄)가 **결정론적으로 조립**한
것이다. terra 산출물은 부분 초안이고 레포에도 그렇게 기록돼 있다
(`fraud_full_rule_ir_terra_partial_output.json`, `fraud_full_rule_ir_terra_failure_audit.json`).
사기 usage의 completion 4,606토큰으로 349 규칙이 나올 수 없다는 것을 견적할 때 알아챘어야 했다.

**따라서 preflight 2항목(단위마다 1콜 생성)과 10항목(추정 $4.52)은 잘못된 전제 위에 있었다.**
RuleIR은 이 규모에서 LLM 단발 생성 과제가 아니고, 검토완료 카드에서 **결정론적으로 컴파일**하는
과제다. LLM은 비평에만 쓴다.

### 조립기 이식 규모 (실측)

사기 조립기는 카드 구동이라 규칙을 손으로 349개 쓴 것이 아니다 — `rule(` 호출 지점 29개,
`predicate(` 18개를 카드 루프에 적용한다. 죄명별로 필요한 것은 **법리 설계 config**다.

| 구성 | 사기 규모 | 성격 |
|---|---:|---|
| `COMPONENT_SOURCES` | 11 component | 구성요건 component → 카드 목록 (실질 법리 작업) |
| `COMPONENT_DEFINITIONS` | 15 | component 정의문 |
| `MODULE_SPECS` | 236줄 | 모듈 분류 + 검수 질문 |
| `BAR_CARD_IDS` | 25줄 | 성립을 막는 카드 목록 |
| `ACTOR_ARGUMENTS` | 6 슬롯 | 이미 preflight에 있음 |

재산죄는 10단위 × component 약 5개 = 50 component와 480장 배정이 필요하다. 레벨 맵(L0~L6)이
그 골격을 이미 주므로 1차 배정은 기계적이고, 단위별로 다듬는 것이 실질 작업이다. **API 0회.**

### 남은 자산은 그대로다

카드 480장·단위 11개·레벨 맵·질의문은 모두 유효하다. 잘못된 것은 "그 다음 단계를 LLM 1콜로
만든다"는 계획 하나이고, 실패 비용은 $0.31이다(누적 $62.7/$100).

## 재산죄 RuleIR 조립 완료 — 사기와 동일 규격, Scallop 런타임 통과 (2026-07-26)

LLM 생성 경로를 버리고 사기 조립 규격을 죄명-파라미터화했다(`scripts/build_property_rule_ir.py`).
규격을 새로 만들지 않았다 — 사기 것이 계약·Scallop 런타임을 통과한 유일한 규격이다. **API 0회.**

| 단위 | 카드 | 술어 | 규칙 | Scallop |
|---|---:|---:|---:|---:|
| robbery | 98 | 213 | 440 | 3,848줄 |
| embezzlement | 64 | 145 | 304 | 2,621줄 |
| theft | 66 | 149 | 303 | 2,654줄 |
| property_damage | 53 | 122 | 253 | 2,186줄 |
| extortion | 41 | 99 | 198 | 1,730줄 |
| breach_of_trust_bribe | 41 | 98 | 197 | 1,718줄 |
| breach_of_trust | 36 | 89 | 171 | 1,515줄 |
| interference_with_exercise_of_right | 32 | 79 | 156 | 1,367줄 |
| lost_property_embezzlement | 14 | 43 | 69 | 647줄 |
| occupational_status | 10 | 32 | 52 | 485줄 |
| **합** | **455** | **1,069** | **2,143** | **18,771줄** |

**10단위 전부 계약 통과.** 산출 `data/rulegen/property/rule_ir/`,
컴파일 `rules/generated/property_*_v1_candidate.scl`.

### 사기 규격과 같은 것

카드마다 `assess_<slug>` 입력 + `satisfied_<slug>` 파생 + "평가가 satisfied이고 provable일 때만
조건으로 승격" 규칙 / 카드마다 undetermined(unknown 보존)·conflict(상반 평가 동시 증명) 규칙 /
component 술어 ← 카드 조건의 인정 경로 / BAR 카드 → `not_established`, 필수 positive의
not_satisfied → 불성립 / `has_negative`·`has_conflict` 2항 요약 /
**`<unit>.core.outcome.established` 단 하나의 규칙에서만 부정** / 시스템 입력 4개.

죄명마다 다른 것은 설정뿐이다. 사기가 손으로 쓴 `COMPONENT_SOURCES`(11 component) 자리에
**레벨 맵**을 넣었다 — 480장을 손으로 매핑하지 않고 이미 만든 L0~L4 배정을 그대로 component로 쓴다.

### 조립 중 걸린 것 3개

1. `standard_input` 카드의 입력 술어는 `kind="standard"`여야 한다(계약). `"rule"`이면
   "standard_input cards without an input standard predicate".
2. 배임수증재는 `defendant_id` 슬롯이 없다(receiver/giver/principal). 브리지 술어 head에
   `defendant_id`를 쓰면 unsafe head variable이 된다 → 첫 역할을 피고인으로 매핑.
3. 업무자 신분(공유 모듈)은 L5가 가중이 아니라 **자기 요건**이다. 죄명 단위에서 L5는 기본범 위에
   얹히는 플래그지만 공유 수정요소에서는 그것이 성립 요건 자체다 → L0으로 읽고 브리지도 배출하지
   않는다(받는 쪽).

### Scallop 런타임 골든 54/54 통과

`scripts/run_property_scallop_golden.py`. scli 0.2.4를 설치하고(체크섬 고정) 단위마다 시나리오를
카드에서 결정론적으로 유도했다 — ordinary_established / incomplete_case_blocked(완결 게이트만 빼면
차단) / negative_bar_blocks / card_conflict_blocks / unknown_blocks(모르면 부정으로 접지 않음) /
aggravation_flag_on(기본범 성립 + 가중 플래그 on). 확인하는 것은 "성립을 함부로 만들지 않고, 모르면
모른다고 하고, 가중을 기본범과 분리한다"는 계약이며 사건 정답 재현 시험이 아니다.

런타임 모듈도 사기 전용이었다 — `ACTOR_FIELDS`(6 슬롯)와 `fraud_case_roles`가 박혀 있어서
RuleIR의 `<issue_tag>_case_roles` 술어에서 역할 계약을 되짚도록 고쳤다(사기 기본값 유지).

회귀 테스트 `tests/test_property_rule_ir.py` 31건 — 조립 결정론(커밋 산출물과 동일), 계약 검증,
컴파일 일치, 런타임 골든, 리포트 최신성. 부정이 최종 결론 규칙 하나에만 있는지도 검사한다.

### 레벨 문서 정정

`rulegen_rule_ir_phases.md` §1의 `established :- ..., not justified`는 계약 위반이었다. L6 카드는
`has_negative`로 흘러가고, 부정은 최종 결론 규칙 하나에서만 쓴다. 조립·런타임으로 확인된 형태로
문서를 고쳤다.

## 재산죄 RuleIR component 재분해 — AND/OR 구조 수정 + 강도 대안트랙 (2026-07-26)

절도 walkthrough(`rulegen_symbolic_layer_theft_walkthrough.md`)에서 "component 내부가 OR이므로
19개 중 하나만 인정되면 객체 요건 전체가 충족된다"는 결함을 발견했다 — 실제로는 소유 **AND**
점유가 둘 다 필요한데 레벨 맵(L0~L4)이 그 AND를 표현 못 했다. 10단위 전수를 카드 단위로 재검토해
고쳤다(API 0회). 판별 기준: **결합적(AND) 요건만 쪼갠다 — 같은 요건의 대안적 인정경로(OR)는
쪼개지 않는다**(예: 손괴죄 객체 카탈로그 18장은 여러 물건유형의 OR라 그대로 둠).

**실제 수정 9건:**
1. 절도 — 객체(소유 AND 점유), 주관(고의 AND 불법영득의사) 분리.
2. 배임 — 주관(고의 AND 불법이득의사) 분리.
3. **강도 — 재물강취(제1항)/이득강취(제2항)를 대안적 실행형태(OR)로 재모델링.** 공유요건(폭행협박·
   인과관계·불법이득의사·착수)은 AND, 트랙 전용요건(객체·기수)은 트랙 안에서만 AND, 트랙끼리는
   OR — 그리고 **두 트랙이 동시에 완전 충족되면 conflict로 잡아 성립을 보류**한다(`UNIT_TRACKS`
   메커니즘 신설, `scripts/build_property_rule_ir.py`. 재사용 가능한 일반 패턴으로 구현했다).
4. 강도 — 재물탈취 시점 인과관계 카드가 "주관"에 오배정 → 인과(L2)로 이동.
5. 강도 — 채무면탈 강도살인 카드가 무소속 "object"에 낙오 → 이득 트랙(L0b)으로 재배정.
6. 횡령 — 객체(보관자지위 AND 타인소유) 분리 + 비자금 카드 3장이 "객체"로 오배정 → 주관(L3)으로.
7. 공갈 — 절 제목("수령할 권리와 불법이득의사") 때문에 착수-단계 카드가 "주관"으로 오배정 → 행위로.
8. 손괴 — 객체 정의 카드가 "행위"로 오배정 → 객체로.
9. 권리행사방해 — 객체(자기소유 AND 타인점유) 분리 + **친족상도례 카드가 기본 객체 요건에
   낙오**(모든 사건에 친족관계 증명을 요구하게 될 뻔) → 절차층(L7)으로 이동.

카드 역할표(`rule_ir_card_roles.json`)도 사용자 지적으로 2건 정정: `art333_sec8` bar→boundary(후속
죄명 폭행/협박 명시), `art366.public_document_used_by_office` bar→boundary(제141조로 전환). 반대로
`art343_sec1.robbery_scope`·`art338_sec4.robbery_death_attempt_excluded`를 bar로 바꾸자는 제안은
근거를 확인하고 반려했다 — 전자는 `aggravation_kinds()`가 역할과 무관하게 L5 카드를 전부 가중
OR-pool에 넣으므로 bar로 바꾸면 그 카드가 만족될 때마다 강도 전체를 저지하는 새 버그가 되고,
후자는 기수/미수 출력이 아직 안 걸려 있어 완성된 강도치사 사건까지 저지할 위험이 있다.

수정마다 재조립→재컴파일→골든 재실행으로 확인했고, 최종 골든 55/55(강도 dual-track conflict
시나리오 1개 신설), pytest 31/31 통과. 조립기(`build_property_rule_ir.py`)·레벨맵 생성기
(`build_rule_ir_phase_map.py`)·카드역할표(`build_rule_ir_card_roles.py`)·골든 시나리오 생성기
(`run_property_scallop_golden.py`)를 모두 고쳤고, 컴파일 스크립트(`build_property_full_scallop.py`)
를 새로 만들어 이전엔 수작업으로 한 번만 컴파일하고 방치돼 있던 `.scl` 산출물의 재현성을 확보했다.

산출: `RuleIR_죄종별_조건식.md`(죄종별 조건식 요약, 사용자 최종검토용), `RuleIR_법리검토.md`(재생성).
